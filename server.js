...
app.post('/api/coinflip/bet', async (req, res) => {
    ...
    try {
        ...
        const isWin = selectedSide === randomResult;
        ...
        if (demoMode) {
            ...
            if (isWin) {
                newBalance = user.demo_balance + betAmount; // Ставка возвращается + выигрыш
            } else {
            ...
            if (isWin) {
                newBalance = user.main_balance + betAmount; // Ставка возвращается + выигрыш
            } else {
            ...
            updateRTPStats('realBank', isWin ? 0 : betAmount, isWin ? betAmount : 0);
        }

        // Сохраняем транзакцию
        transactions.insert({
            user_id: user.$loki,
            amount: isWin ? betAmount : -betAmount,
            type: isWin ? 'coinflip_win' : 'coinflip_loss',
            status: 'completed',
            demo_mode: demoMode,
            game_data: {
                selected_side: selectedSide,
                result_side: randomResult,
                bet_amount: betAmount
            },
            created_at: new Date()
        });
